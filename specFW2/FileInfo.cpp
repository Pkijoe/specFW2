...
$Header: /IcarusBased/SpecFW/FileInfo.cpp 2     8/23/05 10:13 Frazzitl $
...
{
   CString szDrive;//[_MAX_DRIVE];

	szDrive = _tcschr(m_strFilePath , _T( '\\' ) );
...
CString CFileInfo::GetFileDir(void) const { 
   ...
   CString name;
    ...
    name = _tcsrchr(m_strFilePath , _T( '\\' ) );
	szDir = m_strFilePath.Left(m_strFilePath.GetLength() - name.GetLength());
    return CString(szDir);
}

CString CFileInfo::GetFileTitle(void) const 
{
   CString szName;//[_MAX_FNAME];
    // File name follows last slash
    szName = _tcsrchr(m_strFilePath , _T( '\\' ) );
...
{
   CString szExt;//[_MAX_EXT];
    // Extension follows dot
    szExt = _tcsrchr(m_strFilePath , _T( '.' ) );
...
int CFileInfoArray::AddFileInfo(CFileInfo& finf, LPARAM lAddParam) {
   ...
   switch (lAddParam) {
      ...
      case AP_SORTDESCENDING | AP_SORTBYNAME:
         for (i=0;((i<GetSize()) && ((*this)[i].GetFilePath()>finf.GetFilePath()));i++);
      break;
	  // Fixed PR #SITR6BVQDX by adding a default case
	  default:
	  break;
   }
   // Insert it
   ...
   if (hFind != INVALID_HANDLE_VALUE) {
      do {
		if ((wcscmp(wfd.cFileName,_T("."))) && (wcscmp(wfd.cFileName,_T(".."))) && 
		 ...
		 {	// if it is a filename or directory
...
// -------------------------------------------------------------------
// ParsePath
// InfoType is an enum I define in the header
LPTSTR CFileInfo::ParsePath( LPTSTR sPath, int iType )
{
    // TODO: use _tsplitpath
    //  (thanks to Todd Osborn comp.os.ms-windows.programmer.win32 1 Nov 99)
    //  But neither _tsplitpath nor _wsplitpath are defined for Windows
    TCHAR *pStr;
    ...
    case DRIVE:
        // Isolate drive specifier by replacing first slash with null
        // TODO: Fix problem with UNCs (which begin with double backslash)
         pStr = _tcschr( sPath, _T( '\\' ) );
         if (pStr != NULL)
            *pStr = NULL;
        ASSERT( pStr != NULL );
         return sPath;
    ...
    case DIRECTORY:
        // Isolate directory by replacing last slash with null
        pStr = _tcsrchr( sPath, _T( '\\' ) );
         if (pStr != NULL)
            *pStr = NULL;
        ASSERT( pStr != NULL );
        return sPath;
...
$Log: /IcarusBased/SpecFW/FileInfo.cpp $
 * 
 * 2     8/23/05 10:13 Frazzitl
 * Fixed 4 QPR's generated from the code review.
 * 
 * 1     6/08/05 10:21a Frazzitl
